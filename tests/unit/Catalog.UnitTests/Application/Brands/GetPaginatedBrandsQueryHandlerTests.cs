...
namespace Catalog.UnitTests.Application.Brands
{
    public class GetPaginatedBrandsQueryHandlerTests
    {
        ...
        public async Task Handle_Should_ReturnEmptyBrandResponseList_WhenBrandsIsFound_Async()
        {
            ...
            fixture.Freeze<IBrandRepository>().GetPagedBrandsAsync(request.Page, request.Size, request.Keyword, TestContext.Current.CancellationToken).Returns(expected);

            GetPaginatedBrandsQueryHandler sut = fixture.Create<GetPaginatedBrandsQueryHandler>();

            //Act
            PagedList<BrandResponse> result = await sut.Handle(request, TestContext.Current.CancellationToken);
